...
<nav class="navbar navbar-expand-lg navbar-dark sticky-top p-0 custom-navbar">
    <div class="container-fluid px-4">
        ...
        <a href="/home" class="navbar-brand d-flex align-items-center py-3">
            <img src="{{ $branding['assets']['logo_light_url'] ?? ($branding['assets']['logo_url'] ?? '') }}" width="160px" alt="Logo" class="img-fluid">
        </a>
        
        <!-- Título Portal -->
        <div class="d-none d-lg-flex align-items-center mx-auto">
            <h4 class="text-white mb-0 me-4">{{ $branding['texts']['portal_title'] ?? 'Portal de Reclutamiento' }}</h4>
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            ...
            <div class="d-lg-none text-center py-3 border-bottom border-secondary">
                <h5 class="text-white mb-2">{{ $branding['texts']['portal_title'] ?? 'Portal de Reclutamiento' }}</h5>
            ...
            <ul class="navbar-nav ms-auto align-items-lg-center">
                ...
                @else
                    <!-- Authenticated Menu -->
                    @php
                        $isSeguridadProfile = null;
                        foreach ($perfil as $p) {
                            if (($p->perfilesdescrip ?? null) === 'seguridad') {
                                $isSeguridadProfile = true;
                                break;
                            }
                        }
                    @endphp
                    
                    @if (!$isSeguridadProfile)
                    <li class="nav-item">
                        <a href="/home" class="nav-link">
                            <i class="ri-home-4-line me-2"></i>Inicio
                        </a>
                    ...
                    @foreach ($perfil as $pu)
                        @php
                            $isSeguridadProfile = ($pu->perfilesdescrip ?? null) === 'seguridad';
                        @endphp
                        
                        @if ($isSeguridadProfile)
                            <!-- Solo perfil Seguridad: solo muestra opción Seguridad -->
                            <li class="nav-item">
                                <a href="{{ route('seguridad.index') }}" class="nav-link">
                                    <i class="ri-forbid-2-line me-2"></i>Seguridad
                                </a>
                            </li>
                        @else
                            <!-- Gestión Dropdown -->
                            <li class=" dropdown custom-dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end custom-dropdown-menu">
                                    ...
                                    @if (($pu->perfilesdescrip ?? null) === 'admin' || (bool) ($pu->seguridad ?? false))
                                        <li>
                                            <a href="{{ route('seguridad.index') }}" class="dropdown-item">
                    ...
                    @php
                        $allowedBranding = in_array((int) (auth()->user()->perfil_id ?? 0), (array) config('branding.admin_perfil_ids', [1]), true);
                    @endphp
                    @if ($allowedBranding && !$isSeguridadProfile)
                        <li class="nav-item">
                            <a href="{{ route('admin.branding.edit') }}" class="nav-link">
                                <i class="ri-palette-line me-2"></i>Branding
                            </a>
                        </li>
                    @endif
